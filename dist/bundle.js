      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (count2 >= start2) {
          for (let j = i - range; j <= i + range || end > count2; j++) {
            ...
            if (j === i) {
              const pad = start2 - (count2 - lineLength) + 1;
              const length = Math.max(1, end > count2 ? lineLength - pad : end - start2);
...
${expression ? 'Expression: "' + expression + '"\n\n' : ""}`, el);
  ...
  // heroes.json
  var count = 82;
  var next = "https://swapi.dev/api/people/?page=2";
  var previous = null;
  var results = [
    {
      name: "Luke Skywalker",
